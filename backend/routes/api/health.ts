...
router.get("/loadtest/:n", asyncHandler( async (req: Request, res: Response, next: NextFunction) => {
  let n = parseInt(req.params.n);

  let fibonnaci = (n: number) => {
    if (n <= 1) return 1;
// @ts-ignore
...
router.get('/logs/today', asyncHandler( async (req: Request, res: Response, next: NextFunction) => {
  const date = new Date();
...
router.get('/errors/today', asyncHandler( async (req: Request, res: Response, next: NextFunction) => {
  const date = new Date();
  const year = date.getFullYear();
  let month: any = date.getMonth() + 1;
  let day: any = date.getDay();
...
router.get('/logs/week', asyncHandler( async (req: Request, res: Response, next: NextFunction) => {
  ...
  async function processLineByLine(files: any, routes: any) {
...
router.get('/errors/week', asyncHandler( async (req: Request, res: Response, next: NextFunction) => {
...
router.get('/logs/two_week', asyncHandler( async (req: Request, res: Response, next: NextFunction) => {
...
router.get('/errors/two_week', asyncHandler( async (req: Request, res: Response, next: NextFunction) => {
